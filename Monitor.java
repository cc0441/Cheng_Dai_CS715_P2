...
public class Monitor {
    ...
    public void getInLineQ(String name, DataOutputStream dos) {
        Object convey = new Object();
        synchronized (convey) {
            if (cannotGoIn(convey, name)) {
                while (true) //wait to be notified
                    try {
                        msg(name + " waiting for principle to check if test for COVID");
                        dos.writeUTF(name+" waiting for principle to check if test for COVID");
                        convey.wait();
                ...
                msg(name + " is sent to go home by principle.");
            }
    ...
    public void studentGoToNurseRoom(Student students) {
    ...
    public synchronized void studentGoToClassroom(Student students, DataOutputStream dos) throws IOException {
        if (numELA.size() < 4) {

            int roomLeft = 3 - numELA.size();
            msg(students.getName() + " go to ELA and there are " + roomLeft + " room left.");
            dos.writeUTF(students.getName()+" waiting for principle to check if test for COVID");
            ...
            msg(students + " go to Phy.");
            checkWhichClass(students, "PHY");
        }
    }

    public void checkWhichClass(Student students, String className) {
        Object convey = new Object();
        synchronized (convey) {
    ...
    public void wakeUpInstructorAndStudents(Teachers instructor, DataOutputStream dos) {
    }


    public void studentGoToNurseRoom(Student student, DataOutputStream dos) {
    }

    public void getInLineTeacher(Teachers teachers, DataOutputStream dos) {
    }
}
